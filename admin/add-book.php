...
$sql="INSERT INTO book(edition ,b_isbn,category,pub_year,title,pub_name,author_name,copies) VALUES( :edition,:isbn,:category,:publish_year,:bookname,:publisher,:author,:copies)";
...
<div class="form-group">
<label>Publication Year<span style="color:red;">*</span></label>
<input class="form-control" type="text" name="publish_year" autocomplete="off"  required />
</div>

<div class="form-group">
<label>Edition<span style="color:red;">*</span></label>
<input class="form-control" type="text" name="edition" autocomplete="off"  required />
</div>
...
<?php 
$status=1;
$sql = "SELECT CategoryName from category where Status=:status";
$query = $dbh -> prepare($sql);
...
$sql = "SELECT Author_Name from  book_authors ";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
foreach($results as $result)
{               ?>  
<option value="<?php echo htmlentities($result->Author_Name);?>"><?php echo htmlentities($result->Author_Name);?></option>
 <?php }} ?> 
</select>
</div>
